...
warden::ModuleProfile const* Windows12340Profile()
{
    static warden::WardenModuleCatalog const catalog;
    return catalog.Find(12340, "Win");
}
...
warden::CheckPlan TimingMemPlan()
{
    warden::WardenCheckProfile const* profile =
        TestCheckProfile(12340, "enUS");
    warden::CheckPlan plan;
...
{
    warden::CheckPlan plan = TimingPlan();
    plan.checks.push_back(TestDefinition(3));
    return plan;
}
...
{
    warden::ModuleProfile const* profile = Windows12340Profile();
...
TEST(WardenPacket_encodes_exact_12340_module_initialization)
{
    warden::ModuleProfile const* profile = Windows12340Profile();
    REQUIRE(profile != nullptr);
    ...
    CHECK_HEX(initialization.data() + 45, 4, "b4e9d7ba");
}
...
TEST(WardenPacket_encodes_exact_12340_timing_and_mem_request)
{
    warden::ModuleProfile const* profile = Windows12340Profile();
    REQUIRE(profile != nullptr);

    warden::Bytes request{0xA5};
    REQUIRE(warden::EncodeCheckRequest(*profile, TimingMemPlan(), request) ==
        warden::EncodeStatus::Ok);
    CHECK_HEX(request.data(), request.size(),
...
TEST(WardenPacket_encodes_and_decodes_exact_mem_only_confirmation)
{
    ...
    CHECK_HEX(encoded.data(), encoded.size(),
        "02008c00c0a87d00287f");

    warden::CheckBatchResult decoded;
    warden::Bytes const result = FromHex(
        "022900757234ed00"
...
TEST(WardenPacket_rejects_invalid_check_plans_without_replacing_output)
{
    ...
    plan = TimingMpqPlan();
    warden::WardenCheckDefinition duplicate = TestDefinition(3);
    ...
    plan = TimingPlan();
    warden::WardenCheckDefinition oversized = TestDefinition(3);
    std::get<warden::MemCheckProfile>(
...
TEST(WardenPacket_decodes_exact_12340_timing_mem_result_in_plan_order)
{
    ...
    REQUIRE(warden::DecodeCheckResult(View(response), TimingMemPlan(),
    ...
    CHECK_HEX(mem.actualBytes.data(), mem.actualBytes.size(),
        "b9601ad300e8769df9ffe851fbffff688c29af0068d816af00b8b3120000e82d"
        "fdffffa3441ad300");

    warden::Bytes const unavailable =
        FromHex("020600c06da567010403020101");
    REQUIRE(warden::DecodeCheckResult(View(unavailable),
        TimingMemPlan(), result) == warden::DecodeStatus::Ok);
    REQUIRE(result.checks.size() == 2u);
    warden::MemResult const& unavailableMem =
        std::get<warden::MemResult>(result.checks[1]);
    CHECK(unavailableMem.status == warden::MemResultStatus::Unavailable);
    CHECK(unavailableMem.actualBytes.empty());
}
...
{
    warden::Bytes const success = FromHex(
        "021a00b8d19bd3010403020100"
...
TEST(WardenPacket_rejects_malformed_lua_results_without_partial_output)
{
    ...
    warden::Bytes const mpqSuccess = FromHex(
        "021a00b8d19bd3010403020100"
        "d65d59d2e57792a13e8edf78a574b8f81d0d3cf0");
...
TEST(WardenPacket_repeats_mpq_and_lua_definitions_with_shared_strings)
{
    ...
    warden::ModuleProfile const* module = Windows12340Profile();
    ...
    warden::Bytes const response = FromHex(
        "022600c56a29bc0178563412"
        "00d65d59d2e57792a13e8edf78a574b8f81d0d3cf0"
...
TEST(WardenPacket_enforces_client_transport_result_body_budget)
{
    ...
    static_assert(proto::MAX_CLIENT_PACKET_SIZE - sizeof(uint32) -
        ...
        "Wrath Warden CHECK_RESULT body limit must remain exact");
