        ...
        for (auto const& identity :
            {std::pair<warden::WardenCheckType,
             ...
             {warden::WardenCheckType::Mem,
                    warden::WardenEvidenceClass::ThreatSignature}})
        {
            warden::WardenEvidence const mismatch = Evidence(3,
...
{
    warden::WardenEvidence mismatch = Evidence(3,
        warden::WardenCheckType::Mem,
        warden::WardenEvidenceClass::ThreatSignature,
        warden::WardenCheckOutcome::Mismatch);
...
TEST(WardenEnforcementPolicy_confirmation_contract_checks_id_type_and_class)
{
    for (warden::WardenEnforcementMode mode :
        {warden::WardenEnforcementMode::Observe,
            ...
            warden::WardenEnforcementMode::KickAndBan})
    {
        warden::WardenEvidence const pending = Evidence(3,
            warden::WardenCheckType::Mem,
            warden::WardenEvidenceClass::IntegrityInvariant,
            warden::WardenCheckOutcome::Mismatch);
        for (uint32 mutation = 0; mutation < 4; ++mutation)
        {
            ...
            CHECK(decisions[0].checkType == warden::WardenCheckType::Mem);
            CHECK(decisions[0].evidenceClass ==
                warden::WardenEvidenceClass::IntegrityInvariant);
...
{
    warden::WardenEvidence const evidence = Evidence(3,
        warden::WardenCheckType::Mem,
        warden::WardenEvidenceClass::ThreatSignature,
        warden::WardenCheckOutcome::Mismatch);
    for (warden::WardenEnforcementMode mode :
            ...
            warden::WardenEnforcementMode::KickAndBan})
    {
                    ...
                    {evidence, evidence})})
        {
            warden::WardenEnforcementPolicy policy(mode);
            REQUIRE(policy.EvaluateBatch(Batch(
                warden::CheckPlanPurpose::Recurring, {evidence})).size() ==
                1u);
...
TEST(WardenEnforcementPolicy_invalid_confirmation_audits_before_mode_close)
{
    ...
    {
        warden::WardenEnforcementPolicy policy(mode);
        warden::WardenEvidence pending = Evidence(3,
            warden::WardenCheckType::Mem,
            warden::WardenEvidenceClass::IntegrityInvariant,
            warden::WardenCheckOutcome::Mismatch);
        REQUIRE(policy.EvaluateBatch(Batch(
        ...
        // The identity matches exactly, but Stable is illegal for MEM. This
        // catches implementations that erase pending state before classifying
...
TEST(WardenEnforcementPolicy_lifecycle_abort_drains_pending_metadata_without_punishment)
{
    for (warden::WardenEnforcementMode mode :
        {warden::WardenEnforcementMode::Observe,
            warden::WardenEnforcementMode::Kick,
            warden::WardenEnforcementMode::KickAndBan})
    {
        ...
        // The session appends its non-punitive disengagement after persisting
        // this drain. Policy mode and account privilege are deliberately not
        // inputs to this lifecycle operation.
        auto audits = policy.AbortPendingConfirmations();
        ...
        for (warden::WardenPolicyDecision const& audit : audits)
        {
            ...
            sawMpq = sawMpq || (audit.checkId == 1u &&
                audit.checkType == warden::WardenCheckType::Mpq &&
                audit.evidenceClass ==
                    warden::WardenEvidenceClass::Corroboration);
            sawMem = sawMem || (audit.checkId == 3u &&
                audit.checkType == warden::WardenCheckType::Mem &&
                audit.evidenceClass ==
                    warden::WardenEvidenceClass::IntegrityInvariant);
        }
        CHECK(sawMpq);
