...
void instance_blackrock_spire::DoSendNextFlamewreathWave()
{
    ...
    if (m_uiFlamewreathWaveCount < 6)                       // Send two adds (6 waves, then boss)
    {
        ...
        for (uint8 i = 0; i < 2; ++i)
        {
            ...
            pSummoned = pSummoner->SummonCreature((urand(0, 1) && m_uiFlamewreathWaveCount) ? NPC_ROOKERY_GUARDIAN : NPC_ROOKERY_HATCHER, 
                fX, fY, fZ, 0, TEMPSUMMON_TIMED_OOC_OR_DEAD_DESPAWN, 300000);
            if (pSummoned)
            {
