...
bool ChatHandler::HandleAhBotCommand(char* args)
{
    ...
    {
        out << "Module AHBot is disabled";
    }
    else if (strcmp(args, "expire") == 0)
    {
        auctionbot.ExpireAll();
        out << "Auctions expired";
    }
    else if (strcmp(args, "stats") == 0)
    {
        out << auctionbot.PrintAllStats();
    }
    else if (strcmp(args, "update") == 0)
    {
        auctionbot.ActivateNewThread();
        out << "Update started";
    }
    else
    {
        ...
        if(itemId)
        {
            out << auctionbot.LookupItem(itemId);
        }
        else
        {
            out << "auctionbot stats - show short summary\n";
            out << "auctionbot expire - expire all auctions\n";
            out << "auctionbot update - update all auctions\n";
            out << "auctionbot <itemId> - show item price\n";
        }
    }
...
void AhBot::Init()
{
    sLog.outString("Initializing module Ahbot by ike3, based on the original Playerbot by blueboy");
    ...
    sLog.outString("AhBot module loaded");
}
...
void AhBot::ActivateNewThread()
{
    AhbotThread *thread = new AhbotThread(this);
    thread->activate();
}

void AhBot::ExpireAll()
{
...
void AhBot::ForceUpdate()
{
    ...
    sLog.outString("Ahbot module is now checking auctions");
    updating = true;
    ...
    if (!allBidders.size())
    {
        sLog.outError("Ahbot module is disabled but there are no bidders available");
        return;
    }
    ...
    sLog.outString("Ahbot module auction check finished. %d auctions answered, %d new auctions added. Next check in %d seconds",
...
string AhBot::LookupItem(uint32 itemId)
{
...
int AhBot::Answer(int auction, Category* category, ItemBag* inAuctionItems)
{
    ...
    for (vector<AuctionEntry*>::iterator itr = entries.begin(); itr != entries.end(); ++itr)
    {
        ...
        {
            sLog.outDetail("Ahbot module %d won %s (x%d) in auction %d for %d",
        ...
        {
            sLog.outDetail("Ahbot module %d placed bid %d for %s (x%d) in auction %d",
...
int AhBot::AddAuction(int auction, Category* category, ItemPrototype const* proto)
{
    ...
    sLog.outDetail("Ahbot module %d added %d of %s to auction %d for %d..%d", owner, stackCount, proto->Name1, auctionIds[auction], bidPrice, buyoutPrice);
    return 1;
}
...
string AhBot::PrintStats(int auction)
{
    string ahouse = "";
    ...
    else
    {
        ...
        out << auctions.size() << " auctions available in the " << ahouse << " auction house\n";
