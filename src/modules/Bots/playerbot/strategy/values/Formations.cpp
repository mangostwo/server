...
WorldLocation MoveAheadFormation::GetLocation()
{
    ...
    if (master->isMoving())
    {
        ...
        float ground = master->GetMap()->GetHeight(master->GetPhaseMask(), x1, y1, z + 0.5f);
...
namespace ai
{
    ...
    public:
        ...
        virtual WorldLocation GetLocation()
        {
            ...
            float ground = target->GetMap()->GetHeight(target->GetPhaseMask(), x, y, z + 0.5f);
            if (ground <= INVALID_HEIGHT)
            {
                return Formation::NullLocation;
            }
...
WorldLocation MoveFormation::MoveSingleLine(vector<Player*> line, float diff, float cx, float cy, float cz, float orientation, float range)
{
    ...
    for (vector<Player*>::iterator i = line.begin(); i != line.end(); i++)
    {
        ...
        if (member == bot)
        {
            ...
            float lz = cz;
            float ground = bot->GetMap()->GetHeight(bot->GetPhaseMask(), lx, ly, lz + 0.5f);
            if (ground <= INVALID_HEIGHT)
            {
                return Formation::NullLocation;
            }
