...
uint32 GuildTaskMgr::CreateTask(Player* player, uint32 guildId)
{
    switch (urand(0, 1))
    {
    case 0:
        CreateItemTask(player, guildId);
        return GUILD_TASK_TYPE_ITEM;
    default:
        CreateKillTask(player, guildId);
...
bool GuildTaskMgr::CreateItemTask(Player* player, uint32 guildId)
{
    ...
    SetTaskValue((uint32)player->GetGUIDLow(), guildId, "itemCount", count, sPlayerbotAIConfig.maxGuildTaskChangeTime);
    SetTaskValue((uint32)player->GetGUIDLow(), guildId, "itemTask", itemId, sPlayerbotAIConfig.maxGuildTaskChangeTime);
    return true;
}
...
bool GuildTaskMgr::CreateKillTask(Player* player, uint32 guildId)
{
        ...
        if (!co)
  {
    ...
    SetTaskValue((uint32)player->GetGUIDLow(), guildId, "killTask", creatureId, sPlayerbotAIConfig.maxGuildTaskChangeTime);
    return true;
}
