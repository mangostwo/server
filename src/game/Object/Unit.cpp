...
void Unit::WriteMovementInfo(ByteBuffer& out) const
{
    ...
    if (!vessel)
    {
        ...
        ashore.Report(Where().X(), Where().Y(), Where().Z(), Where().Facing());
        ashore.Write(out);
        return;
    }
    ...
    onDeck.SetTransportData(vessel->GetObjectGuid(), Where().X(), Where().Y(), Where().Z(),
                            Where().Facing(), 0, -1);
    onDeck.Report(0.0f, 0.0f, 0.0f, Where().Facing());
    onDeck.Write(out);
}
 ...
 */
float CombatReachBetween(Unit const& attacker, Unit const& victim, bool forMeleeRange, float flat_mod)
{
...
Unit* Unit::SelectMagnetTarget(Unit* victim, Spell* spell, SpellEffectIndex eff)
{
    ...
    // Magic case
    if (spell && spell->m_spellInfo->DefenseType == SPELL_DAMAGE_CLASS_MAGIC)
    {
...
void Unit::SetDeathState(DeathState s)
{
    ...
    if (s == JUST_DIED)
    {
        ...
        // Unboard from a VEHICLE. GetTransport() is a WorldObject, and on a ship it is the
        // vessel -- a GameObject. Casting that to Unit* and reading IsVehicle() walks off
        // the end of the object; a deckhand dying aboard crashed the world thread here.
        if (TransportInfo* boarded = GetTransportInfo())
        {
            if (boarded->IsOnVehicle())
            {
                Unit* carrier = static_cast<Unit*>(boarded->GetTransport());
                if (carrier->IsVehicle())
                {
                    carrier->RemoveSpellsCausingAura(SPELL_AURA_CONTROL_VEHICLE, GetObjectGuid());
                }
            }
        }
...
int32 Unit::CalculateAuraDuration(SpellEntry const* spellProto, uint32 effectMask, int32 duration, Unit const* caster)
{
    ...
    {
        dispelMod   = GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_DURATION_OF_EFFECTS_BY_DISPEL, spellProto->DispelType);
        dmgClassMod = GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_DURATION_OF_MAGIC_EFFECTS, spellProto->DefenseType);
    }
    ...
    if (caster == this)
    {
        switch (spellProto->SpellClassSet)
        {
            case SPELLFAMILY_DRUID:
                // Thorns
                if (spellProto->SpellIconID == 53 && (spellProto->SpellClassMask & UI64LIT(0x0000000000000100)))
                {
                    // Glyph of Thorns
            ...
            case SPELLFAMILY_PALADIN:
                // Blessing of Might
                if (spellProto->SpellIconID == 298 && spellProto->SpellClassMask & UI64LIT(0x0000000000000002))
                {
                ...
                // Blessing of Wisdom
                else if (spellProto->SpellIconID == 306 && spellProto->SpellClassMask & UI64LIT(0x0000000000010000))
                {
...
{
    float angle = this == target ? Where().Facing() + M_PI_F : target->Where().BearingTo(Where());
...
void Unit::KnockBackWithAngle(float angle, float horizontalSpeed, float verticalSpeed)
{
    ...
    else
    {
        ...
        float ox, oy, oz;
        ox = Where().X(), oy = Where().Y(), oz = Where().Z();
