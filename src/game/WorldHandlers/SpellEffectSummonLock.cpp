...
void Spell::EffectSummonType(SpellEffectIndex eff_idx)
{
    ...
    {
        summonPositions[0].x = realCaster->Where().X(), summonPositions[0].y = realCaster->Where().Y(), summonPositions[0].z = realCaster->Where().Z();
    ...
    for (++itr; itr != summonPositions.end(); ++itr)        // In case of multiple summons around position for not-fist positions
    {
        ...
        {
            const Geometry::Vector3 spot = RandomGroundPointNear(
                *realCaster, Geometry::Vector3(summonPositions[0].x, summonPositions[0].y, summonPositions[0].z), radius);
            itr->x = spot.x;
            itr->y = spot.y;
            itr->z = spot.z;
        ...
        {
            ClosePointNear(*realCaster, itr->x, itr->y, itr->z, 0.0f, radius, frand(0.0f, 2 * M_PI_F));
            if (realCaster->GetMap()->GetHitPosition(summonPositions[0].x, summonPositions[0].y, summonPositions[0].z, itr->x, itr->y, itr->z, m_caster->GetPhaseMask(), -0.5f))
            {
                ClampToAllowedZ(*realCaster, itr->x, itr->y, itr->z);
            }
...
bool Spell::DoSummonPossessed(CreatureSummonPositions& list, SummonPropertiesEntry const* prop, SpellEffectIndex effIdx, uint32 level)
{
    ...
    Creature* spawnCreature = m_caster->SummonCreature(creatureEntry, list[0].x, list[0].y, list[0].z, m_caster->Where().Facing(), TEMPSPAWN_CORPSE_DESPAWN, 0);
    if (!spawnCreature)
    {
