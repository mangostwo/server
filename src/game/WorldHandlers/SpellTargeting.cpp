...
void Spell::SetTargetMap(SpellEffectIndex effIndex, uint32 targetMode, UnitList& targetUnitMap)
{
    ...
    switch (targetMode)
    {
        ...
        case TARGET_TOTEM_FIRE:
        {
            ...
            { const Geometry::Vector3 p = PointNear(*m_caster, radius, angle); x = p.x; y = p.y; }
        ...
        case TARGET_DYNAMIC_OBJECT_RIGHT_SIDE:
        {
            if (!(m_targets.m_targetMask & TARGET_FLAG_DEST_LOCATION))
            {
                ...
                float x, y;
                { const Geometry::Vector3 p = PointNear(*m_caster, radius + m_caster->Where().Extent(), angle); x = p.x; y = p.y; }
        ...
        case TARGET_POINT_AT_SW:
        {
            if (!(m_targets.m_targetMask & TARGET_FLAG_DEST_LOCATION))
            {
                ...
                float x, y;
                { const Geometry::Vector3 p = PointNear(*currentTarget, radius + currentTarget->Where().Extent(), angle); x = p.x; y = p.y; }
