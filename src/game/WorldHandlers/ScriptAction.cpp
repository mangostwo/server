    ...
    switch (m_script->command)
    {
            ...
            switch (m_script->movement.movementType)
            {
                ...
                case RANDOM_MOTION_TYPE:
                    ...
                    else
                    {
                        ...
                        const Geometry::Placement& spawnPose = ((Creature*)pSource)->Spawn();
                        respX = spawnPose.X();
                        respY = spawnPose.Y();
                        respZ = spawnPose.Z();
                        respO = spawnPose.Facing();
                                                wander_distance = ((Creature*)pSource)->GetRespawnRadius();;
        ...
        case SCRIPT_COMMAND_MOVE_DYNAMIC:                   // 37
        {
            ...
            else                                            // Calculate position
            {
                ...
                x = spot.x;
                y = spot.y;
                z = std::max(spot.z, pTarget->Where().Z());
