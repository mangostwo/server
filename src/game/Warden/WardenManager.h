...
public:
    ...
    /** Atomically publishes one normalized configuration for future sessions. */
    void PublishConfiguration(WardenConfiguration configuration);
    /** Returns one non-null coherent immutable configuration snapshot. */
    std::shared_ptr<WardenConfiguration const> GetConfigurationSnapshot() const;
...
private:
    ...
    std::shared_ptr<WardenConfiguration const> m_configuration;
};
}
