...
warden::ModuleProfile const ModuleWin12340 =
{
    // Module identities and rekey values belong to the delivered module. The
    // callback RVAs below are independently recovered from exact build 12340.
    12340,
    "Win",
    {warden::WardenModuleWinData, warden::WardenModuleWinSize},
...
{
    // Do not infer compatibility from shared module bytes: callback RVAs and
    // future absolute checks remain exact to build 12340.
...
{
    if (profile.module.size != WardenModuleWinSize)
    ...
    if (CRYPTO_memcmp(md5.data(), profile.moduleId.data(), md5.size()) != 0 ||
        CRYPTO_memcmp(sha256.data(), profile.moduleSha256.data(),
            sha256.size()) != 0)
        return ModuleValidation::DigestMismatch;
