...
{
    // The exact pre-Cataclysm generator is verified by the signed client module:
    // hash each 20-byte half, then expand SHA1(left || previous || right).
...
void WardenCryptoContext::Rc4State::Initialize(Key16 const& key)
{
    ...
    {
        swapIndex =
            (swapIndex + permutation[index] + key[index % key.size()]) & 0xFF;
