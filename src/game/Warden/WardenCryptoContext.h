...
namespace warden
{
/** Owns the independent streaming RC4 directions for one Warden session. */
...
public:
    ...
    // Derives both directions without retaining the authenticated session key.
    bool Initialize(SessionKey const& sessionKey);
    bool IsInitialized() const;

    // Each transform advances its directional stream across packet boundaries.
    bool TransformClientToServer(Bytes& bytes);
    bool TransformServerToClient(Bytes& bytes);

    // Parsing operates on a temporary stream copy. Moving an accepted clone
    // back commits it; destroying a rejected clone rolls the stream back.
