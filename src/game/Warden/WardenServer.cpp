...
void WardenServer::HandleEncrypted(ByteView encryptedBody)
{
    ...
    // Decrypt against a temporary stream. Malformed input discards the
    // advanced candidate; only a fully decoded message commits it.
    WardenCryptoContext crypto = m_crypto.CloneForTransaction();
    if (!crypto.TransformClientToServer(plain))
    {
        Fail(WardenFailure::CryptoFailure);
        return;
    }
    ...
    {
        HandleCheckResult(plain, std::move(crypto));
        return;
    }
...
bool WardenServer::SendPlain(Bytes plain)
{
    ...
    // Keep the live stream unchanged unless the complete encrypted body is
    // accepted by the transport callback.
    WardenCryptoContext crypto = m_crypto.CloneForTransaction();
    if (!crypto.TransformServerToClient(plain))
    {
        Fail(WardenFailure::CryptoFailure);
        return false;
    }
